...
- [x] **Evidence**: Verified in `src/frontend/components/investigation/` and passed in `test_api_catalog.py`.
...
- [x] **Evidence**: Verified in `src/frontend/components/map/` and Next.js static export build.
...
- [x] **Evidence**: Verified in `src/frontend/components/charts/` and `src/frontend/components/evidence/`.
...
- [x] **Evidence**: All 63 backend tests passed (`pytest terra-odyssey/tests/unit/ -v`). Next.js static build exported cleanly (`out/`). Root SPA served at `GET /` (HTTP 200).
