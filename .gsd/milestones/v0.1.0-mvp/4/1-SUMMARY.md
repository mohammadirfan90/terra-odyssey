...
1. **Extended InvestigationRecord JSON Schema**:
   - Updated `terra-odyssey/schemas/investigation-record.schema.json` to formally type `schema_version`, `job` (with `job_status`, `stage`, `result_status`, `progress_pct`, and timestamps), `data_mode`, `artifact_index`, `record_hash`, `published_at`, `source_manifest_objects`, and `resolved_configuration_hash`.
   - Verified clean compilation with `jsonschema.Draft202012Validator`.

2. **Pydantic v2 Request & Response Models**:
   - Created `terra-odyssey/src/backend/schemas.py` with strict Pydantic v2 models:
...
3. **RFC 9457 Problem Details Error Handlers**:
   - Implemented `terra-odyssey/src/backend/errors.py` providing custom exceptions (`TerraOdysseyError`, `DataUnavailableError`, `ScientificallyIneligibleError`, `InvalidGeometryError`, `InvestigationNotFoundError`, `InvestigationConflictError`) and FastAPI exception handlers delivering `application/problem+json`.

4. **Catalog and Capabilities Endpoints**:
   - Implemented `GET /api/catalog` and `GET /api/capabilities` in `terra-odyssey/src/backend/api/catalog.py`.
   - Built FastAPI app factory in `terra-odyssey/src/backend/app.py` with CORS, routing, and error handling.

5. **Empirical Verification**:
   - Created unit tests in `terra-odyssey/tests/unit/test_api_catalog.py`.
...
- `terra-odyssey/src/backend/__init__.py`
- `terra-odyssey/src/backend/schemas.py`
- `terra-odyssey/src/backend/errors.py`
- `terra-odyssey/src/backend/api/__init__.py`
...
- `pytest terra-odyssey/tests/unit/test_api_catalog.py`: 4/4 passed
- `pytest terra-odyssey/tests/unit/`: 53/53 passed
