...
from src.backend.errors import TerraOdysseyError

logger = logging.getLogger("terra_odyssey.backend.exporter")


def _find_schema_path() -> Path:
    """Locate schemas/investigation-record.schema.json across runtime environments."""
    candidates = [
        Path(__file__).resolve().parents[3] / "schemas" / "investigation-record.schema.json",
        Path.cwd() / "terra-odyssey" / "schemas" / "investigation-record.schema.json",
        Path.cwd() / "schemas" / "investigation-record.schema.json",
    ]
    for p in candidates:
        if p.is_file():
            return p
